...
use crate::{consistent_hash_vault_assignment, ClaimLeaf, ClaimProofV1};
...
impl ClaimTreeV1 {
    ...
    fn generate_proof(&self, leaf_index: usize) -> Result<Vec<Vec<[u8; 32]>>> {
        ...
        while current_hashes.len() > 1 {
            ...
            let chunk_index = current_index / WIDTH;
            let position_in_chunk = current_index % WIDTH;

            // Get all chunks at this level
            let chunks: Vec<&[[u8; 32]]> = current_hashes.chunks(WIDTH).collect();
    ...
    pub fn depth(&self) -> usize {
        ...
        while remaining > 1 {
            remaining = (remaining + 255) / 256; // Ceiling division
            depth += 1;
        }
...
mod tests {
    ...
    fn test_claim_proof_v1_verify_single_leaf() {
        ...
        assert_eq!(root, leaf.to_hash(), "Single leaf root should equal leaf hash");
        assert!(proof.verify(&root, &leaf), "Single leaf proof should verify");
    }
        ...
        for &size in &test_sizes {
            ...
            if !leaves.is_empty() {
                ...
                if size == 1 {
                    ...
                    assert!(proof.len() > 0, "Multi-leaf tree should have non-empty proof");
                    assert!(proof.total_hashes() > 0, "Multi-leaf proof should have hashes");
                }
            }
        }
    }
    ...
    fn test_claim_proof_v1_verify_cross_tree_isolation() {
        ...
        let leaves_tree_a = vec![
            create_test_leaf(1, 100),
            create_test_leaf(2, 200),
        ];
        
        let leaves_tree_b = vec![
            create_test_leaf(3, 300),
