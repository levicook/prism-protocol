use prism_protocol_sdk::build_activate_cohort_v0_ix;
...
/// Test that cohort activation is NOT permissionless
///
...
///
/// They still CANNOT activate the cohort because PDA derivation uses the admin's key.
/// The instruction will fail with AccountNotInitialized, proving the security model works.
#[ignore]
#[test]
fn test_non_admin_cannot_activate_cohort() {
    ...
    match result {
        Ok(_) => {
            panic!("❌ Cohort activation should have failed - instruction is not permissionless!");
        }
        Err(failed_meta) => {
            ...
            match failed_meta.err {
                TransactionError::InstructionError(_, InstructionError::Custom(code)) => {
                    ...
                    println!("✅ This proves cohort activation is NOT permissionless");
                }
                _ => {
                    panic!(
    ...
    let (correct_ix, _, _) = build_activate_cohort_v0_ix(
        &test.state.address_finder,
