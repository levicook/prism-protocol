...
#[test]
fn test_no_vaults_expected_error() {
    ...
    let test_fingerprint = generate_test_fingerprint("test_no_vaults_expected_error");
    let merkle_root = [1u8; 32];

    // Try to initialize cohort with 0 expected vaults (should fail)
    let (initialize_cohort_ix, ix_accounts, _) = build_initialize_cohort_v0_ix(
        ...
        0,             // expected_vault_count = 0 (invalid)
    )
    ...
    // This should fail with NoVaultsExpected error
    ...
    println!("✅ No vaults expected error correctly triggered");
}
