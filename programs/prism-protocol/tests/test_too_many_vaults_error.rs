...
fn test_too_many_vaults_error() {
    ...
    let test_fingerprint = generate_test_fingerprint("test_too_many_vaults_error");
    let merkle_root = [1u8; 32];

    // Try to initialize cohort with too many vaults (let's assume max is 255)
    let (initialize_cohort_ix, ix_accounts, _) = build_initialize_cohort_v0_ix(
        ...
        255, // expected_vault_count = max u8, but might still be too many based on our limits
    )
    ...
    // This might fail with TooManyVaults error if we have limits set
    // For now, let's just see if it succeeds or what error we get
    let result = fixture.mollusk.process_and_validate_instruction(
        ...
        &[Check::err(ProgramError::Custom(
            PrismError::TooManyVaults as u32,
        ))],
    );

    println!("✅ High vault count handled appropriately");
}
